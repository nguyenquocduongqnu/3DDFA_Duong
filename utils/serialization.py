...
def ser_to_obj(img, ver_lst, tri, height):
    """convert serialize output to vertices, faces, vertex colors"""
    n_vertex = ver_lst[0].shape[1]
    n_face = tri.shape[0]
    vertex_colors = []
    vertices = []
    ...
    for i in range(len(ver_lst)):
        ...
        for j in range(n_vertex):
            x, y, z = ver[:, j]
            vertices.append([round(x,2),round(height-y,2),z])
            vertex_colors.append([round(colors[j, 2],2),round(colors[j, 1],2),round(colors[j, 0],2)])

        offset = i * n_vertex
        for j in range(n_face):
            idx1, idx2, idx3 = tri[j]  # m x 3
            faces.append([idx3 + offset,idx2 + offset,idx1 + offset])
